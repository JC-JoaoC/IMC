...
function resultadoFormulario(event) {
  ...
  const imcTotal = (inputPeso / (inputAltura * inputAltura)).toFixed(2);
   ...
   if (imcTotal >= 0.00 && imcTotal <= 18.49){
    resultado.innerHTML = `<p> &#x1F626 Seu IMC é de: <strong> ${imcTotal} </strong> - Isso significa que você é muito magro.  </p>`
  }
  
   else if (imcTotal >= 18.50 && imcTotal <= 24.99) {
    resultado.innerHTML = `<p> &#x1F604 Seu IMC é de: <strong> ${imcTotal} </strong> - Seu IMC é considerado "Normal".  </p>`
  }
  
    else if (imcTotal >= 25.00 && imcTotal <= 29.99) {
    resultado.innerHTML = `<p> &#x1F615  Seu IMC é de: <strong> ${imcTotal} </strong> - Isso significa que sua classificação é "Sobrepeso".  </p>`
  }
    ...
    em um terceiro grau de obesidade. Thais Carla? </p>`
  }
